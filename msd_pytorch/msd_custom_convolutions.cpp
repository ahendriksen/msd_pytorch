...
torch::Tensor conv3d_forward(torch::Tensor input,
    ...
    // Check same device (only for forward pass)
    torch::checkAllSameGPU("conv3d_forward", {arg_input, arg_kernel, arg_bias, arg_output});
    ...
    // Check dimensions
    AT_ASSERTM(input.dim() == 5, "input must be five-dimensional");
    AT_ASSERTM(kernel.dim() == 5, "kernel must be five-dimensional");
    AT_ASSERTM(output.dim() == 5, "output must be five-dimensional");
    AT_ASSERTM(bias.dim() == 1, "bias must be one-dimensional");

    // Check that kernel is 3x3x3:
    ...
    AT_ASSERTM(input.size(2) == output.size(2), "Input and output shape do not match");
    AT_ASSERTM(input.size(3) == output.size(3), "Input and output shape do not match");
    AT_ASSERTM(input.size(4) == output.size(4), "Input and output shape do not match");

    return conv3d_cuda_forward(input, kernel, bias, output, dilation, block_size);
}

void conv3d_backward_x(torch::Tensor grad_output,
		       torch::Tensor kernel,
		       torch::Tensor grad_input,
		       int dilation,
		       int block_size) {
    ...
    // check dims
    AT_ASSERTM(grad_output.dim() == 5, "grad_output must be five-dimensional");
    AT_ASSERTM(kernel.dim() == 5, "kernel must be five-dimensional");
    AT_ASSERTM(grad_input.dim() == 5, "grad_input must be five-dimensional");
    ...
    AT_ASSERTM(grad_input.size(2) == grad_output.size(2), "Grad_input and grad_output shape do not match");
    AT_ASSERTM(grad_input.size(3) == grad_output.size(3), "Grad_input and grad_output shape do not match");
    AT_ASSERTM(grad_input.size(4) == grad_output.size(4), "Grad_input and grad_output shape do not match");

    conv3d_cuda_backward_x(grad_output, kernel, grad_input, dilation, block_size);
}

void conv3d_backward_k(torch::Tensor grad_output,
		       torch::Tensor input,
		       torch::Tensor grad_kernel,
		       int dilation,
		       int block_size) {
    ...
    // check dims
    AT_ASSERTM(grad_output.dim() == 5, "grad_output must be five-dimensional");
    AT_ASSERTM(input.dim() == 5, "grad_kernel must be five-dimensional");
    AT_ASSERTM(grad_kernel.dim() == 5, "grad_kernel must be five-dimensional");
    ...
    AT_ASSERTM(input.size(2) == grad_output.size(2), "Grad_input and grad_output shape do not match");
    AT_ASSERTM(input.size(3) == grad_output.size(3), "Grad_input and grad_output shape do not match");
    AT_ASSERTM(input.size(4) == grad_output.size(4), "Grad_input and grad_output shape do not match");

    conv3d_cuda_backward_k(grad_output, input, grad_kernel, dilation, block_size);
}

void conv3d_backward_bias(torch::Tensor grad_output,
			  torch::Tensor grad_bias,
			  int block_size) {
...
torch::Tensor conv3d_relu_forward(torch::Tensor input,
				  torch::Tensor kernel,
				  torch::Tensor bias,
				  torch::Tensor output,
				  int dilation,
    ...
    // Check same device (only for forward pass)
    torch::checkAllSameGPU("conv3d_relu_forward", {arg_input, arg_kernel, arg_bias, arg_output});
    ...
    return conv3d_relu_cuda_forward(input, kernel, bias, output, dilation, block_size);
}

void conv3d_relu_backward_x(torch::Tensor output,
                          torch::Tensor grad_output,
                          torch::Tensor kernel,
                          torch::Tensor grad_input,
                          int dilation,
    ...
    conv3d_relu_cuda_backward_x(output, grad_output, kernel, grad_input, dilation, block_size);
}

void conv3d_relu_backward_k(torch::Tensor output,
                          torch::Tensor grad_output,
                          torch::Tensor input,
                          torch::Tensor grad_kernel,
                          int dilation,
    ...
    conv3d_relu_cuda_backward_k(output, grad_output, input, grad_kernel, dilation, block_size);
}

void conv3d_relu_backward_bias(torch::Tensor output,
                             torch::Tensor grad_output,
                             torch::Tensor grad_bias,
                             int block_size) {
    ...
    conv3d_relu_cuda_backward_bias(output, grad_output, grad_bias, block_size);
}
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m) {
    ...
    m.def("conv3d_forward", &conv3d_forward, "Forward convolution"
          "input"_a, "kernel"_a, "bias"_a, "output"_a, "dilation"_a,
          "block_size"_a=8);
    m.def("conv3d_backward_x", &conv3d_backward_x, "Transpose of the forward convolution",
          "grad_output"_a, "kernel"_a, "grad_input"_a, "dilation"_a,
          "block_size"_a=4);
    m.def("conv3d_backward_k", &conv3d_backward_k, "Transpose of the forward convolution",
          "grad_output"_a, "input"_a, "grad_kernel"_a, "dilation"_a,
          "block_size"_a=8);
    m.def("conv3d_backward_bias", &conv3d_backward_bias, "Backward bias",
          "grad_output"_a, "grad_bias"_a,
          "block_size"_a=8);

    m.def("conv3d_relu_forward", &conv3d_relu_forward, "Forward convolution"
          "input"_a, "kernel"_a, "bias"_a, "output"_a, "dilation"_a,
          "block_size"_a=8);
    m.def("conv3d_relu_backward_x", &conv3d_relu_backward_x, "Transpose of the forward convolution",
          "output"_a, "grad_output"_a, "kernel"_a, "grad_input"_a, "dilation"_a,
          "block_size"_a=4);
    m.def("conv3d_relu_backward_k", &conv3d_relu_backward_k, "Transpose of the forward convolution",
          "output"_a, "grad_output"_a, "input"_a, "grad_kernel"_a, "dilation"_a,
          "block_size"_a=8);
    m.def("conv3d_relu_backward_bias", &conv3d_relu_backward_bias, "Backward bias",
          "output"_a, "grad_output"_a, "grad_bias"_a,
          "block_size"_a=8);
