...
template <typename scalar_t>
__global__ void
conv_backward_bias0(dTensor4R grad_output,
		    dTensor1R grad_bias)
{
...
template <typename scalar_t>
__global__ void
conv_backward_k0(dTensor4R grad_output,
		 dTensor4R input,
...
template <typename scalar_t>
__global__ void
conv_backward_k1(dTensor4R grad_output,
		 dTensor4R input,
		 dTensor4R grad_kernel,
		 int dilation)
{
...
template <typename scalar_t>
__global__ void
conv_backward_x0(dTensor4R grad_output,
		dTensor4R kernel,
...
template <typename scalar_t>
__global__ void
conv_backward_x1(dTensor4R grad_output,
		 dTensor4R kernel,
		 dTensor4R grad_input,
		 int dilation)
{
...
at::Tensor conv_cuda_forward(at::Tensor input_t,
	...
	if (implementation == 2) {
	    conv2<scalar_t><<<gridSize, blockSize, buffer_sz>>>
		(input_d, kernel_d, bias_d, out_d, dilation);
	} else if (implementation == 3) {
	    conv3<scalar_t><<<gridSize, blockSize, buffer_sz>>>
		(input_d, kernel_d, bias_d, out_d, dilation);
	} else if (implementation == 4) {
	    conv4<scalar_t><<<gridSize, blockSize, buffer_sz>>>
		(input_d, kernel_d, bias_d, out_d, dilation);
	} else if (implementation == 5) {
	    conv5<scalar_t><<<gridSize, blockSize, buffer_sz>>>
		(input_d, kernel_d, bias_d, out_d, dilation);
	}
...
void conv_cuda_backward_x(at::Tensor grad_output_t,
   ...
   AT_DISPATCH_FLOATING_TYPES(grad_output_t.type(), "conv_cuda_backward_x", ([&] {
	...
	dTensor4R kernel_d = toDeviceTensorR<scalar_t,4>(kernel_t);
    	...
    	auto buffer_sz = kernel_t.numel() * sizeof(scalar_t);
	if (implementation == 0) {
	    conv_backward_x0<scalar_t><<<gridSize, blockSize, buffer_sz>>>
		(grad_output_d, kernel_d, grad_input_d, dilation);
	} else if (implementation == 1) {
	    conv_backward_x1<scalar_t><<<gridSize, blockSize, buffer_sz>>>
		(grad_output_d, kernel_d, grad_input_d, dilation);
	}
    	THCudaCheck(cudaGetLastError());
    }));
			  ...
			  int dilation, int implementation, int block_size)
{
   AT_DISPATCH_FLOATING_TYPES(grad_output.type(), "conv_cuda_backward_k", ([&] {
	...
	dTensor4R grad_kernel_d = toDeviceTensorR<scalar_t,4>(grad_kernel);
        ...
        dim3 blockSize(block_size, block_size);
	if (implementation == 0) {
	    conv_backward_k0<scalar_t><<<gridSize, blockSize>>>
		(grad_output_d, input_d, grad_kernel_d, dilation);
	} else if (implementation == 1) {
	    conv_backward_k1<scalar_t><<<gridSize, blockSize>>>
		(grad_output_d, input_d, grad_kernel_d, dilation);
	}
...
void conv_cuda_backward_bias(at::Tensor grad_output,
			     at::Tensor grad_bias,
			     int implementation, int block_size)
{
   AT_DISPATCH_FLOATING_TYPES(grad_output.type(), "conv_cuda_backward_bias", ([&] {
	...
	dTensor1R grad_bias_d = toDeviceTensorR<scalar_t,1>(grad_bias);
        ...
        dim3 blockSize(block_size, block_size);
	if (implementation == 0) {
	    conv_backward_bias0<scalar_t><<<gridSize, blockSize>>>
		(grad_output_d, grad_bias_d);
	}
