...
from msd_pytorch.conv2d import (
    Conv2dModule,
    conv2d,
    conv2d_forward,
    conv2d_backward_x,
...
def test_conv2d():
    ...
    ci = Conv2dModule(output, in_channels, 1, kernel_size=kernel_size)
...
def test_dtype_check():
    ...
    with pytest.raises(RuntimeError):
        conv2d_forward(x, k, bias, y, dilation)
...
def test_grad_check():
    ...
    def f(x, k, b, dilation):
        ...
        stride = 1
        return conv2d(x, k, b, output, stride, dilation)
...
def test_conv_backward_x():
    ...
    def AT(y, k):
        ...
        conv2d_backward_x(y, k, x, dilation)
        return x
...
def test_conv_backward_k():
    ...
    def AT(y, x):
        ...
        conv2d_backward_k(y, x, k_grad, dilation)
        return k_grad
...
def test_conv_backward_bias():
    ...
    conv2d_backward_bias(grad, g_bias)
...
def test_conv_relu_dtype_check():
    """Test if dtype checks are performed correctly
    """
    d0 = torch.float
    d1 = torch.double
    ...
    with pytest.raises(RuntimeError):
        conv2d_relu_forward(x, k, bias, y, dilation)
...
def test_conv2d_relu_grad_check():
    """Test using gradcheck provided by Torch.
    ...
    def f(x, k, b, dilation):
        ...
        output = x.new_zeros(output_shape, requires_grad=True)
        stride = 1
        return conv2d_relu(x, k, b, output, stride, dilation)
